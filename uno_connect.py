...
# Status
state, player_name, error_message = "name_input", "", ""
ws_thread, connected_players, card_rects = None, [], []
    ...
    if state == "name_input":
            ...
            for player in GameStatus.players:
                if player["name"] != player_name:
                    for i in range(player["no_of_cards"]):
                        x, y = (300 + (i % 9) * 70) / 2, (HEIGHT / 2 - 140 + (i // 9) * 110) / 2
                        pygame.draw.rect(screen, BLACK, pygame.draw.rect(screen, GRAY, (x, y, 30, 45)), 1)
        except:
            pass
        ...
        try:
            if len(GameStatus.your_handcards) == 0:
                draw_text("Du hast gewonnen!", pygame.Rect(0, 50, WIDTH, 50))
        except:
            pass
                    ...
                    for rect, idx in card_rects:
                        if rect.collidepoint(event.pos):
                            try:
                                ...
                                pass
                            if top_card and (card.color == top_card.color or card.value == top_card.value or top_card.color == "black" or card.color == "black"):
                                play_card(card)
                            else:
